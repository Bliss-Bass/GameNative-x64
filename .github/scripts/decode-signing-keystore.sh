...
if [ -z "${SIGNING_KEY:-}" ]; then
  echo "::error::SIGNING_KEY repository secret is not set. Set it to: base64 -w0 <your-release.jks>"
  exit 1
fi
...
if [ ! -s "$OUTPUT_PATH" ]; then
  echo "::error::Decoded keystore is empty. Update SIGNING_KEY using: base64 -w0 release.jks"
  exit 1
fi

if ! file "$OUTPUT_PATH" | grep -qE 'Java KeyStore|OpenPGP|data'; then
